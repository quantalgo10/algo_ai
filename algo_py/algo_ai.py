        ...
        try:
            ...
            if login_response.get('status'):
                ...
                if profile.get('status'):
                    st.success(f"Login successful for client: {self.client_id}")
                    return True
    ...
    if not trade_df.empty:
        ...
        with tab1:
            ...
            if not closed_trades.empty:
                closed_trades['Exit Time'] = pd.to_datetime(closed_trades['Exit Time'])
                closed_trades['Entry Time'] = pd.to_datetime(closed_trades['Entry Time'])
